...
1. **Question 6 Correction:**
    - **Original Issue:** The original option C, *Parthenium* (Carrot grass), is incorrect as *Parthenium* is not a grass but a weed from the Asteraceae family.
    - **Action Taken:** Updated all options to include actual grasses known to cause pollen allergies in India and corrected the answer key accordingly.
    - **New Options:**
        - **A.** \textit{Cynodon dactylon} (Bermuda grass)
        - **B.** \textit{Sorghum halepense} (Johnson grass)
        - **C.** \textit{Phleum pratense} (Timothy grass)
        - **D.** \textit{Festuca pratensis} (Meadow fescue)
    - **Correct Answer:** Changed from C to A.

2. **Consistency in Answer Key:**
    - Updated the answer for Question 6 in the answer key from C to A to reflect the corrected options.

3. **Formatting Adjustments:**
    - Ensured that percentages in options are properly escaped using `\%`.
    - Italicized scientific names using `\textit{}` for proper LaTeX formatting.

All other questions and sections were reviewed and found to be accurate and clear, hence they were preserved without changes.